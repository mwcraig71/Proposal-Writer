...
- **Multi-User Authentication:** Role-based access control with three roles: Admin (full access + user management), Editor (full access to data), and Viewer (read-only + downloads). Uses username/password login with hashed passwords (Werkzeug). Admins manage users at `/admin/users`. The `before_request` hook enforces auth globally and blocks write operations for viewers. An admin user is auto-seeded on first startup from `APP_PASSWORD`.
